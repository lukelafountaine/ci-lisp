...
// helper for defining a function
SYMBOL_AST_NODE* arg_list(char* symbol, SYMBOL_AST_NODE* arg_list)
{
  ...
  //make the type real because we dont support typed parameters
  p->type = REAL;
  // give it the name
  p->name = symbol;
  ...
  if (arg_list)
  {
    // insert the new symbol at the end of the arg_list
    SYMBOL_AST_NODE* current = arg_list;
    SYMBOL_AST_NODE* previous = current;
    // iterate to the end and place the new arg
    while (current)
    {
      previous = current;
      current = current->next;
...
AST_NODE* s_expr_list(AST_NODE* s_expr_list, AST_NODE* s_expr)
{
  ...
  // just adds the s_expr to the end of the list
  if (s_expr_list)
  {
    // insert the new symbol at the end of the arg_list
    AST_NODE* current = s_expr_list;
    AST_NODE* previous = current;
    // iterate to the end and place the new s_expr
    while (current)
    {
      previous = current;
      current = current->next;
    }
    previous->next = s_expr;
    s_expr = s_expr_list;
  }
